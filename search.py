...
def run_train(experiment_id, r_max, forces_weight, energy_weight, max_num_epochs,
    ...
    tracking_client.log_batch(run_id=p.run_id, metrics=[],
                             params=[Param("r_max", str(r_max)),
                                 Param("max_num_epochs", str(max_num_epochs)),
                                 Param("forces_weight", str(forces_weight)),
                                 Param("energy_weight", str(energy_weight))],
...
@click.option("--max_num_epochs", type=click.INT, default=100, help="Maximum number of epochs to train.")
...
def run(num_runs, max_num_epochs,train_backend_config, train_file, valid_file):
    ...
    with mlflow.start_run(run_id=provided_run_id) as run:
        ...
        for r_max, forces_weight in runs:
            jobs.append(run_train(
                experiment_id,
                r_max=r_max, forces_weight=forces_weight, energy_weight=1.0,
